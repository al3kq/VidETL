...
pipeline = None
out_dir = ""
in_dir = pipeline_data["directory"]

out_dir = all_data["output_directory"]
name = pipeline_data["name"]
for task in pipeline_data["tasks"]:
    tasks.append(create_editor(task))

pipeline = VideoEditingPipeline(name=name, tasks=tasks)

clip = execute_pipeline_from_dir(in_dir, pipeline)
save_video_files(clip, out_dir)