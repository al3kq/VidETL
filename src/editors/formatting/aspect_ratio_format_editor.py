...
class AspectRatioFormatter(BaseFormatEditor):
    def __init__(self, video_clip, target_aspect_ratio):
    ...
    def apply_format(self):
        ...
        if target_width <= original_width:
            ...
            cropped_clip = self.video_clip.crop(x1=crop_x_start, y1=0, width=target_width, height=original_height)
        else:
            target_height = original_width / self.target_aspect_ratio
            crop_y_start = (original_height - target_height) / 2
            cropped_clip = self.video_clip.crop(x1=0, y1=crop_y_start, width=original_width, height=target_height)
